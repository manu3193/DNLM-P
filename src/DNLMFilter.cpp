...
int DNLMFilter::dnlmFilterBW(const Npp32f* pSrcBorder, int stepBytesSrcBorder, const Npp32f* pUSMImage, int stepBytesUSM, Npp32f* pDst, int stepBytesDst, NppiSize imageSize, int w, int w_n, float sigma_r){
    ...
    //Set buffer to 0
    nppiSet_32f_C1R((Ipp32f) 0.0f, pWeightsAcumm, stepBytesWeightsAcumm, imageSize);
    ...
    //Init buffer to 0
    nppiSet_32f_C1R((Ipp32f) 0.0f, pChunkMem, stepBytesThreadWeights, {imageSize.width, 2*imageSize.height});
    ...
    {   
        ...
        for (int dm = 0; dm < wHalfLen+1; ++dm)
        {
            ...
            if (dn>0 || (dn==0 && dm>0))
            {
                ...
                nppiSet_32f_C1R((Ipp32f) 0.0f, pEuclDist, stepBytesEuclDist, euclROISize);
                  
                //Compute squared diference 
                nppiSub_32f_C1R((Ipp32f*) (pSrcBorder +indexSrcImageBaseWOffset + (m_min + dm)), stepBytesSrcBorder, 
                ...
                //Compute weights
                nppiDivC_32f_C1IR((Ipp32f) -(2*sigma_r * sigma_r), pEuclDist, stepBytesEuclDist, euclROISize);
                nppiExp_32f_C1IR(pEuclDist, stepBytesEuclDist, euclROISize);          

                //Performing filtering
                nppiMul_32f_C1R(pEuclDist, stepBytesEuclDist, (Ipp32f*) (pUSMImage +indexUSMImageBaseWOffset + (m_min + dm)), stepBytesUSM, pTmp, stepBytesTmp, euclROISize);
                //Exploiting weights symmetry
                nppiMul_32f_C1R(pEuclDist, stepBytesEuclDist, (Ipp32f*) (pUSMImage + indexUSMImageBase + m_min), stepBytesUSM, pTmp2, stepBytesTmp, euclROISize);
                      
                //Accumulate signal and weights
                nppiAdd_32f_C1IR(pTmp, stepBytesTmp, (Ipp32f*) (pThreadDst + indexDstBase + m_min), stepBytesThreadDst, euclROISize);
                nppiAdd_32f_C1IR(pEuclDist, stepBytesEuclDist, (Ipp32f*) (pThreadWeightsAcumm + indexWeightsAcummBase + m_min), stepBytesThreadWeights, euclROISize);
                //Accumulate signal and weights 
                nppiAdd_32f_C1IR(pTmp2, stepBytesTmp, (Ipp32f*) (pThreadDst + indexDstBaseWOffset + (m_min + dm)), stepBytesThreadDst, euclROISize);
            ...
            {   
                ...
                nppiAddC_32f_C1IR((Ipp32f) 1.0f, pThreadWeightsAcumm, stepBytesThreadWeights, imageSize );
